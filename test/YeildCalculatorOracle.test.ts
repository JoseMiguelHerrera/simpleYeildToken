...
import {
  ...
  describe("YeildCalculatorOracle", function () {
    ...
    describe("Setters", function () {
        describe("Success", function () {
          it("Oracle is deployed successfully", async function () {
            expect(await yeildCalculatorOracle.token()).to.be.equal(simpleYeildToken.target)
            expect(await yeildCalculatorOracle.growthPerSecond()).to.be.equal(bpsToGrowthPerSecond(initialBps))
          })

          it("Apr can be changed successfully", async function () {
            const newBps=700;
            await yeildCalculatorOracle.changeBps(newBps);
            expect(await yeildCalculatorOracle.growthPerSecond()).to.be.equal(bpsToGrowthPerSecond(newBps))
          })
        })
        ...
        describe("Failure", function () {
            it("Can't change bps - not owner", async function () {
                const newBps=700;
                await expect((yeildCalculatorOracle.connect(user1) as Contract).changeBps(newBps)).to.revertedWith("Ownable: caller is not the owner")
              })
        })

    })
    describe("Yeild factor gets updated correctly", function () {

        it("Yeild factor increases correctly (60 seconds without update)", async function () {
            //apr is 5%
            ...
            await yeildCalculatorOracle.updateYeildFactor();
        ...
        it("Yeild factor increases correctly (one hour without update)", async function () {
            //apr is 5%
            const initYeildFactor = await simpleYeildToken.yeildFactor();
        ...
        it("Yeild factor increases correctly (one year without update)", async function () {
            //apr is 5%
            const initYeildFactor = await simpleYeildToken.yeildFactor();
        ...
        it("Yeild factor increases correctly (with a apr change in between)", async function () {
            //apr is 5%
            const initYeildFactor = await simpleYeildToken.yeildFactor();
        ...
        it("Yeild factor increases correctly due to APR change", async function () {
            //apr is 5%
            const initYeildFactor = await simpleYeildToken.yeildFactor();
